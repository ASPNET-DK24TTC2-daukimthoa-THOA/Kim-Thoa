...
namespace TiemTraSua.Areas.Admin.Controllers
{
    ...
    public class BillController : Controller
    {
        ...
        public IActionResult Index(int? page)
        {
            ...
            var listItem = _context.TbHoaDonBans.AsNoTracking().Include(x => x.Customer).OrderByDescending(x => x.NgayBan).ToList();
        ...
        public IActionResult Details(int? page, string id, string name)
        {
            int pageSize = 30;
            int pageNumber = page == null || page < 0 ? 1 : page.Value;
            var listItem = _context.TbChiTietHoaDonBans.AsNoTracking().Include(x => x.MaSanPhamNavigation).Where(x => x.MaHoaDon == Guid.Parse(id)).OrderBy(x => x.MaHoaDon).ToList();
