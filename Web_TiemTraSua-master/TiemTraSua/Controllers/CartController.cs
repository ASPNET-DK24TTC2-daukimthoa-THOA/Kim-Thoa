...
namespace TiemTraSua.Controllers
{
    public class CartController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            {
                ViewData["Message"] = "Không có s?n ph?m trong gi? hàng.";
                ViewData["total"] = "0";
                return View(Carts);
            }
        }
        ...
        public IActionResult Update([FromBody] List<UpdateQuantityRequest> updates)
        {
            ...
            if (cartItems != null)
            {
                ...
                return Json(new { success = true, message = "S? lu?ng dã du?c c?p nh?t.", totalAmount = totalAmount, cartItems = cartItems });
            }
            ...
            try
            {
                ...
                if (cartItems != null)
                {
                    ...
                    if (productToRemove != null)
                    {
                        ...
                        return Json(new { success = true, message = "Ðã xoá s?n ph?m.", totalAmount = totalAmount, cartItems = cartItems });
                    }
                    else
                    {
                        Console.WriteLine(maSp);
                        return Json(new { success = false, message = "Không có s?n ph?m." });
                    }
                }
                else
                {
                    return Json(new { success = false, message = "Gi? hàng r?ng." });
                }    
            }
            catch (Exception ex)
            {
